<script lang="ts">
  import { Download, Share2, Trash2, Upload } from '@lucide/svelte';
...
<section class="mx-auto max-w-4xl px-4 py-6 sm:px-6">
  <Panel>
    ...
    <div class="mt-5 flex flex-wrap gap-3">
      <button class="inline-flex items-center gap-2 rounded-md bg-teal-500 px-3 py-2 text-sm font-medium text-neutral-950 hover:bg-teal-400 disabled:opacity-60" type="button" disabled={exportBusy} on:click={handleExport}>
        <Download size={16} /> {exportBusy ? 'Exporting…' : 'Export JSON'}
      </button>
    ...
    <h3 class="mt-6 text-sm font-semibold text-neutral-50">Share a read-only copy</h3>
    <p class="mt-2 text-sm text-neutral-400">Export a single self-contained HTML file anyone can open in a browser — no install, no account, works offline. It's a read-only, item-by-item viewer with the same flip-card and photo gallery experience.</p>
    <div class="mt-3">
      <button class="inline-flex items-center gap-2 rounded-md border border-teal-700 px-3 py-2 text-sm font-medium text-teal-300 hover:bg-neutral-800 disabled:opacity-60" type="button" disabled={shareBusy} on:click={handleShare}>
        <Share2 size={16} /> {shareBusy ? 'Preparing…' : 'Export shareable HTML'}
      </button>
    </div>
